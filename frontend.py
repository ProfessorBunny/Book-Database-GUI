...
def get_selected_row(event):
    ...
    # try and cath block is implemented to prevent any error from occuring when user select(tap) empty list box
    try:
        global selected_tuple
...
def delete_command():
    ...
    # now call view command, so that we can see the updated db in the list box
    view_command()
...
window.wm_title("Bunny's BookStore")
...
# This ma1es sure to keep the main window open
window.mainloop()
